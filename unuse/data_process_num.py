...
class DataProcess(object):
    ...
    def __init__(self):
        ...
        self.image_path = '/data/data/train_1000/image_1000'
        self.text_path = '/data/data/train_1000/txt_1000'
        self.json_path = '/data/CRNN_draft/train_data_num/json/meta.json'
        self.json_val_path = '/data/CRNN_draft/train_data_num/json/val.json'
        self.save_path = '/data/CRNN_draft/train_data_num/cut_images'
        self.key_path = '/data/CRNN_draft/train_data_num/key/keys.txt'

        self.img_height = 64
        self.img_width = 128

        # init directory
        if not os.path.exists('/data/CRNN_draft/train_data_num'):
            os.mkdir('/data/CRNN_draft/train_data_num')
        if not os.path.exists('/data/CRNN_draft/train_data_num/json'):
            os.mkdir('/data/CRNN_draft/train_data_num/json')
        if not os.path.exists('/data/CRNN_draft/train_data_num/key'):
            os.mkdir('/data/CRNN_draft/train_data_num/key')
        if not os.path.exists('/data/CRNN_draft/train_data_num/cut_images'):
            os.mkdir('/data/CRNN_draft/train_data_num/cut_images')
        ...
        for root, dirs, files in os.walk(self.image_path):
            for file in files:
                ...
                with open(text_file, encoding='utf-8') as f:
                    for line_index, labels in enumerate(f.readlines()):

                        if 'backside' in file and line_index not in [5]:
                            continue
                        labels = labels.strip('\r\n')
                        arr = labels.split(',')
                        ...
                        if 'frontage' in file and not (line_index==6 or label_length==18):
                            continue
